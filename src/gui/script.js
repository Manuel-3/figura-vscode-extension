...
// Check href and src for allowed domains and only https and remove some additional attributes
DOMPurify.addHook('beforeSanitizeAttributes', node => {
    ...
    if (node.tagName === 'IMG') {
        ...
        if (!allowedDomains.includes(srcurl.hostname) || !src.startsWith('https:')) {
            // Remove src if the domain is not allowed or not https
    ...
    if (node.tagName === 'A') {
        ...
        if (!allowedDomains.includes(hrefurl.hostname) || !href.startsWith('https:')) {
            // Remove href if the domain is not allowed or not https
    ...
    switch (data.command) {
        case 'returnLongDescriptionMarkdown':
            element.innerHTML = DOMPurify.sanitize(marked.parse(data.longdescription), {
                ALLOWED_TAGS: ['b', 'i', 'em', 'strong', 'a', 'p', 'ul', 'ol', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'img', 'blockquote', 'code', 'pre', 'table', 'td', 'tr', 'tbody', 'thead', 'th', 'hr', 'details', 'summary', 'span'], // 'input', 
                ALLOWED_ATTR: ['href', 'src', 'class', 'alt' ], // 'type', 'disabled', 'checked'
            });
            return;
...
async function listFiles() {
    ...
    querySnapshot.forEach((doc) => {
        ...
        https://chatgpt.com/share/6717c5d6-a908-8010-9914-5eb7ff915b3e
        https://chatgpt.com/share/6717c60f-1f4c-8010-9133-c8f7d411a6d1
        https://chatgpt.com/share/6717c62b-e800-8010-bb76-572b15b98833
        https://chatgpt.com/share/6717c6a8-5f3c-8010-9ed2-5aa5146e11d9

        const imageElement = clone.querySelector('.package-image');
        const imageUniqueId = crypto.randomUUID();
        imageElement.setAttribute('data-unique-id', imageUniqueId);
        vscode.postMessage({ command: 'downloadImage', url: `https://firebasestorage.googleapis.com/v0/b/fpm-figura-package-manager.appspot.com/o/images%2F${encodeURIComponent(data.uid)}%2F${encodeURIComponent(data.username)}.${encodeURIComponent(data.title)}.png?alt=media`, callbackUniqueId: imageUniqueId });
*/
